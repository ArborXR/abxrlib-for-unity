...
public class AuthenticationTests : AbxrPlayModeTestBase
{
    ...
    // ── MDM / SSO authentication (ArborServiceClient disabled in tests) ────

    [Test]
    public void GetIsAuthenticated_WithoutArborServiceClient_ReturnsFalse()
        => Assert.IsFalse(Abxr.GetIsAuthenticated());

    [Test]
    public void GetAccessToken_WithoutArborServiceClient_ReturnsEmpty()
    {
    ...
    [Test]
    public void GetRefreshToken_WithoutArborServiceClient_ReturnsEmpty()
    {
    ...
    public void SetDeviceId_Null_ClearsOverride()
    {
        ...
        Assert.IsTrue(id == null || id == "", "Clearing override should fall back to empty (no ArborServiceClient)");
    }
    ...
    public void SetOrgId_Null_ClearsOverride()
    {
        Abxr.SetOrgId("my-org-uuid");
        Abxr.SetOrgId(null);
        // Falls back to Configuration.orgID which is "" in test config
    ...
    {
        // When UserId is null/empty, the subsystem derives it from UserData fields.
