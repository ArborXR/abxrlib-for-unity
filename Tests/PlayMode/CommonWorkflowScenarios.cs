...
{
    /// <summary>Prefix for test console output so you can see each Event/Log call. Filter Console by "AbxrLib (Test)" to see only scenario logs.</summary>
    private const string T = "[AbxrLib] (Test)";
    ...
    public IEnumerator FullAssessmentWithObjectivesAndInteractions_Scenario()
    {
        ...
        Debug.Log(T + " EventObjectiveComplete: " + objectiveName + " score=" + objectiveScore + " Pass");
        Abxr.EventObjectiveComplete(objectiveName, objectiveScore, Abxr.EventStatus.Pass);
        Debug.Log(T + " Log Debug: Objective completed (meta: score)");
        ...
        Debug.Log(T + " EventAssessmentComplete: Assessment_" + randomName + " score=" + assessmentScore + " Pass (meta: max_score=100)");
    ...
    public IEnumerator ExperienceOnly_NoScore_Scenario()
    {
        ...
        Debug.Log(T + " EventExperienceStart: " + experienceName);
        Abxr.EventExperienceStart(experienceName);
        yield return new WaitForSeconds(0.2f);

        Debug.Log(T + " Event: content_selected (meta: content_type=video, content_id=safety_intro_01, source=gallery)");
        ...
        Debug.Log(T + " LogInfo: Loading experience content (meta: content_id)");
        Abxr.LogInfo("Loading experience content", new Abxr.Dict().With("content_id", "safety_intro_01"));
        yield return new WaitForSeconds(0.3f);
        Debug.Log(T + " Log Info: Experience loaded successfully (meta: duration_ms, format)");
        Abxr.Log("Experience loaded successfully", Abxr.LogLevel.Info, new Abxr.Dict().With("duration_ms", "420").With("format", "360"));
        yield return new WaitForSeconds(0.2f);

        Debug.Log(T + " Event: viewer_position position=(0,1.6,-2) meta: event=playback_started");
        Abxr.Event("viewer_position", new Vector3(0f, 1.6f, -2f), new Abxr.Dict().With("event", "playback_started"));
        yield return new WaitForSeconds(0.25f);
        Debug.Log(T + " Event: viewer_position position=(0.2,1.55,-1.9) meta: event=mid_playback");
        Abxr.Event("viewer_position", new Vector3(0.2f, 1.55f, -1.9f), new Abxr.Dict().With("event", "mid_playback"));
        yield return new WaitForSeconds(0.25f);

        Debug.Log(T + " StartTimedEvent: video_watch_safety_intro_01");
        Abxr.StartTimedEvent("video_watch_safety_intro_01");
        yield return new WaitForSeconds(0.5f);
        Debug.Log(T + " Event: video_watch_safety_intro_01 (duration from StartTimedEvent, meta: content_id, completed)");
        ...
        Debug.Log(T + " EventExperienceComplete: " + experienceName);
    ...
    public IEnumerator AssessmentFail_Scenario()
    {
        ...
        Debug.Log(T + " LogInfo: Quiz started");
        Abxr.LogInfo("Quiz started", new Abxr.Dict().With("assessment", assessmentName));
        Debug.Log(T + " EventAssessmentStart: " + assessmentName);
        Abxr.EventAssessmentStart(assessmentName);
        yield return new WaitForSeconds(0.2f);

        Debug.Log(T + " EventObjectiveStart: q1_ppe_required (Q1)");
        Abxr.EventObjectiveStart("q1_ppe_required_" + tag);
        Debug.Log(T + " EventInteractionStart: select_ppe_answer");
        Abxr.EventInteractionStart("select_ppe_answer_" + tag);
        yield return new WaitForSeconds(0.2f);
        Debug.Log(T + " EventInteractionComplete: select_ppe_answer Select Correct \"gloves_and_goggles\"");
        ...
        Debug.Log(T + " EventObjectiveComplete: q1 score=100 Pass");
        Abxr.EventObjectiveComplete("q1_ppe_required_" + tag, 100, Abxr.EventStatus.Pass);
        yield return new WaitForSeconds(0.15f);

        Debug.Log(T + " EventObjectiveStart: q2_emergency_exit (Q2)");
        Abxr.EventObjectiveStart("q2_emergency_exit_" + tag);
        Debug.Log(T + " EventInteractionStart: select_exit_answer");
        Abxr.EventInteractionStart("select_exit_answer_" + tag);
        yield return new WaitForSeconds(0.2f);
        Debug.Log(T + " EventInteractionComplete: select_exit_answer Select Incorrect \"nearest_door\"");
        ...
        Debug.Log(T + " EventObjectiveComplete: q3 score=0 Fail");
        Abxr.EventObjectiveComplete("q3_fire_extinguisher_" + tag, 0, Abxr.EventStatus.Fail);
        yield return new WaitForSeconds(0.15f);

        Debug.Log(T + " Log Info: Assessment complete – below passing threshold (meta: score=25)");
        Abxr.Log("Assessment complete – below passing threshold", Abxr.LogLevel.Info, new Abxr.Dict().With("score", "25"));
        Debug.Log(T + " EventAssessmentComplete: " + assessmentName + " score=25 Fail");
    ...
    public IEnumerator BasicEventWithMetadata_Scenario()
    {
        ...
        Debug.Log(T + " Event: button_pressed (meta: screen=main_menu, button=start)");
        Abxr.Event("button_pressed", new Abxr.Dict().With("screen", "main_menu").With("button", "start"), sendTelemetry: false);
        yield return new WaitForSeconds(0.15f);
        Debug.Log(T + " Event: scene_entered (meta: scene_id=warehouse, entry_point=spawn_a)");
        Abxr.Event("scene_entered", new Abxr.Dict().With("scene_id", "warehouse").With("entry_point", "spawn_a"), sendTelemetry: false);
        yield return new WaitForSeconds(0.2f);
        Debug.Log(T + " Event: item_collected (meta: item_type=coin, item_value=100, location=shelf_1)");
        ...
        Debug.Log(T + " Event: item_collected (meta: item_type=tool, item_value=wrench, location=bench_2)");
        ...
        yield return new WaitForSeconds(0.2f);
        Debug.Log(T + " Event: player_teleported position=(2,0,-1) meta: destination=checkpoint_b");
        Abxr.Event("player_teleported", new Vector3(2f, 0f, -1f), new Abxr.Dict().With("destination", "checkpoint_b"));
        yield return new WaitForSeconds(0.15f);
        Debug.Log(T + " Event: button_pressed (meta: screen=inventory, button=use_item)");
        Abxr.Event("button_pressed", new Abxr.Dict().With("screen", "inventory").With("button", "use_item"), sendTelemetry: false);
        Debug.Log(T + " LogInfo: User completed inventory action");
    ...
    public IEnumerator TimedAssessment_Scenario()
    {
        ...
        Debug.Log(T + " StartTimedEvent: " + assessmentName + " (duration will attach to EventAssessmentComplete)");
        Abxr.StartTimedEvent(assessmentName);
        Debug.Log(T + " EventAssessmentStart: " + assessmentName);
        Abxr.EventAssessmentStart(assessmentName);
        Debug.Log(T + " LogInfo: Timed exam started");
        Abxr.LogInfo("Timed exam started", new Abxr.Dict().With("assessment", assessmentName));
        yield return new WaitForSeconds(0.2f);

        Debug.Log(T + " EventObjectiveStart: question_1");
        Abxr.EventObjectiveStart("question_1_" + tag);
        Debug.Log(T + " EventInteractionComplete: answer_1 Select Correct \"a\"");
        Abxr.EventInteractionComplete("answer_1_" + tag, Abxr.InteractionType.Select, Abxr.InteractionResult.Correct, "a");
        Debug.Log(T + " EventObjectiveComplete: question_1 score=100 Pass");
        Abxr.EventObjectiveComplete("question_1_" + tag, 100, Abxr.EventStatus.Pass);
        yield return new WaitForSeconds(0.25f);
        Debug.Log(T + " EventObjectiveStart: question_2");
        Abxr.EventObjectiveStart("question_2_" + tag);
        Debug.Log(T + " EventInteractionComplete: answer_2 Select Correct \"b\"");
        Abxr.EventInteractionComplete("answer_2_" + tag, Abxr.InteractionType.Select, Abxr.InteractionResult.Correct, "b");
        Debug.Log(T + " EventObjectiveComplete: question_2 score=100 Pass");
        Abxr.EventObjectiveComplete("question_2_" + tag, 100, Abxr.EventStatus.Pass);
        yield return new WaitForSeconds(0.3f);
        Debug.Log(T + " Log Info: Timed exam completed (meta: score=88)");
        Abxr.Log("Timed exam completed", Abxr.LogLevel.Info, new Abxr.Dict().With("score", "88"));
        Debug.Log(T + " EventAssessmentComplete: " + assessmentName + " score=88 Pass (includes duration from StartTimedEvent)");
    ...
    public IEnumerator SuperMetadataRegister_Scenario()
    {
        ...
        yield return new WaitForSeconds(1f);
        Debug.Log(T + " Register: user_type=premium, app_version=1.2.3 (super metadata on all subsequent events/logs)");
        Abxr.Register("user_type", "premium");
        Abxr.Register("app_version", "1.2.3");
        Debug.Log(T + " RegisterOnce: user_tier=free (set only if not already set)");
        Abxr.RegisterOnce("user_tier", "free");
        yield return new WaitForSeconds(0.1f);

        Debug.Log(T + " Event: session_started (meta: source=main_menu)");
        Abxr.Event("session_started", new Abxr.Dict().With("source", "main_menu"), sendTelemetry: false);
        yield return new WaitForSeconds(0.15f);
        Debug.Log(T + " LogInfo: Premium session active");
        Abxr.LogInfo("Premium session active", new Abxr.Dict().With("feature", "extended_content"));
        Debug.Log(T + " Event: content_selected position=(0,1.5,-2) meta: content_id=premium_video_1");
        Abxr.Event("content_selected", new Vector3(0f, 1.5f, -2f), new Abxr.Dict().With("content_id", "premium_video_1"));
        yield return new WaitForSeconds(0.2f);
        Debug.Log(T + " Event: playback_started (meta: content_id=premium_video_1)");
    ...
    public IEnumerator StorageSetAndGet_User_Scenario()
    {
        ...
        Debug.Log(T + " Clear device-scoped storage so user-scope GET does not see leftover device data.");
        ...
        Debug.Log(T + " ForceSend storage/events, then wait 1.5s for backend to persist. (If no Storage POST log appears, user-scope add was skipped due to no userId.)");
        ...
        Debug.Log(T + " StorageGetEntry: name=state scope=User (async, wait for callback)");
        ...
        AbxrSubsystem.Instance.StartCoroutine(Abxr.StorageGetEntry("state", Abxr.StorageScope.User, r =>
        {
            results = r;
            getDone = true;
            Debug.Log(T + " StorageGetEntry callback: results=" + (r == null ? "null" : "List.Count=" + r.Count.ToString()));
            if (r != null && r.Count > 0)
            {
                var first = r[0];
                var keys = first != null ? string.Join(", ", first.Keys) : "(null)";
                Debug.Log(T + " StorageGetEntry first entry keys: " + keys);
            }
        }));
        yield return new WaitUntil(() => getDone);
        Debug.Log(T + " StorageGetEntry completed: results " + (results == null ? "null" : "Count=" + results.Count.ToString()));
        Assert.IsNotNull(results, "StorageGetEntry callback should be invoked (null = request failed or error).");
        Assert.Greater(results.Count, 0, "Backend should return at least one entry for 'state' (scope=User); ensure backend is running and storage POSTs completed (check [AbxrLib] Storage POST logs).");
        Debug.Log(T + " StorageGetEntry returned " + results.Count + " entry/entries.");
        Debug.Log(T + " LogInfo: Progress loaded for resume");
    ...
    public IEnumerator StorageSetAndGet_Device_Scenario()
    {
        ...
        Debug.Log(T + " LogInfo: Device storage session started");
        Abxr.LogInfo("Device storage session started", new Abxr.Dict().With("session", tag));
        Debug.Log(T + " Event: device_checkpoint (meta: checkpoint=intro, device_progress=25%)");
        Abxr.Event("device_checkpoint", new Abxr.Dict().With("checkpoint", "intro").With("device_progress", "25%"), sendTelemetry: false);
        Debug.Log(T + " StorageSetEntry: name=state scope=Device (device_progress=25%, device_last_checkpoint=intro)");
        ...
        Debug.Log(T + " Event: device_checkpoint position=(1,0,2) meta: checkpoint=mid");
        Abxr.Event("device_checkpoint", new Vector3(1f, 0f, 2f), new Abxr.Dict().With("checkpoint", "mid"));
        Debug.Log(T + " StorageSetEntry: name=state scope=Device (device_progress=75%, device_last_checkpoint=mid)");
        ...
        Debug.Log(T + " Log Debug: Device progress saved (meta: device_progress=75%)");
        Abxr.Log("Device progress saved", Abxr.LogLevel.Debug, new Abxr.Dict().With("device_progress", "75%"));
        yield return new WaitForSeconds(0.15f);

        Debug.Log(T + " ForceSend storage/events, then wait 1.5s for backend to persist.");
        AbxrSubsystem.Instance.GetTransportForTesting()?.ForceSend();
        yield return new WaitForSeconds(1.5f);

        Debug.Log(T + " StorageGetEntry: name=state scope=Device (async, wait for callback)");
        List<Dictionary<string, string>> results = null;
        bool getDone = false;
        ...
        Debug.Log(T + " StorageGetEntry returned " + results.Count + " entry/entries (device scope).");
        Debug.Log(T + " LogInfo: Device progress loaded for resume");
