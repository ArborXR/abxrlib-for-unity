...
- **Auth Handoff Launcher:** In **Advanced ArborXR Settings**, **Enable Auth Handoff Launcher** (`enableLearnerLauncherMode`, default false). When enabled, after config is received the SDK overrides the auth mechanism: if the API’s authMechanism type is not already **assessmentPin**, it is set to **assessmentPin** with prompt **"LMS PIN"**. Use when a launcher collects PIN before **`Abxr.StartAuthentication()`**, registers an **OnInputRequested** handler that does nothing (to suppress the built-in keyboard), then calls **StartAuthentication()** and **`Abxr.OnInputSubmitted(enteredValue)`** when ready. The normal flow then runs (OnInputRequested is invoked with type pin; app submits via OnInputSubmitted).
