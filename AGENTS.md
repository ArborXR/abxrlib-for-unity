...
- **Invalid `auth_handoff`:** If the intent/CLI/WebGL parameter is present but the payload cannot be normalized or applied, the SDK logs a warning and continues with **normal device authentication** (same as if no handoff were provided).
- **Valid auth handoff (receiving app):** After applying the launcher session, the SDK still runs **GET /v1/storage/config** (or the ArborInsightsClient equivalent) so **Configuration** is updated from the server for non-auth settings. The receiver is **not** prompted again for PIN/email from that config (identity already came from the handoff).
- **GET config fails after successful device auth:** The SDK logs a warning, treats **authMechanism** as **none** for this session (no keyboard), and **completes authentication** so the app can continue; other values remain whatever is already in the Unity **Configuration** asset.
...
- **`Abxr.OnAuthCompleted`:** For integration, subscribing only to **`Abxr.OnAuthCompleted(bool success, string errorMessage)`** is enough to know when the SDK has finished the authentication sequence (success or failure). The subsystem forwards **all** terminal auth failures through the same path (including validation errors, device auth, config handling, and wrong PIN while re-prompting), so you do **not** need a separate subscription to the auth service’s internal **`OnFailed`** unless you want an extra hook.
...
- **Quit / EndSession auto-close:** Any assessment, objective, or interaction still open is completed with meta `quit_reason=application_quit` and `auto_closed=true`. Override the recorded score/status via **`Abxr.OnQuitAssessmentStatus`**, **`OnQuitAssessmentScore`**, **`OnQuitObjectiveStatus`**, **`OnQuitObjectiveScore`**, **`OnQuitInteractionType`**, **`OnQuitInteractionResult`**, **`OnQuitInteractionResponse`**, and optional **`OnQuitInteractionScore`** (when non-null, adds meta `score` on interactions). **`AbxrSubsystem.ResetStaticStateForTesting()`** restores these to SDK defaults.
