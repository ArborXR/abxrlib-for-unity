...
- **AuthMechanism (second-stage tests):** `RuntimeAuthConfig.authMechanism` is the single auth mechanism (type, prompt, domain). When null or empty type, it is filled from GET config when received; when set by tests before config is fetched, that value is used and not overwritten. Tests set `authMechanism` so each test gets the second-stage flow it needs (assessmentPin, email, text, none). Default prompts when type is set: email → "Enter your email address", text → "Enter your Employee ID"; assessmentPin uses "Enter your 6-digit PIN".
