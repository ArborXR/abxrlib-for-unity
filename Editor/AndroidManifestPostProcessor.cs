...
namespace AbxrLib.Editor
{
    ...
    public class AndroidManifestPostProcessor : IPostGenerateGradleAndroidProject
    {
        ...
        /// Returns the buildType field value, or "production" as default. Production (Custom APK) returns "production" for manifest metadata compatibility.
        /// </summary>
        ...
        private static void ValidateAppTokensForBuild()
        {
            ...
            if (configData.useAppTokens)
            {
                if (!string.IsNullOrEmpty(configData.appToken) && !LooksLikeJwt(configData.appToken))
                    throw new BuildFailedException("App Token is set but does not look like a JWT (expected three dot-separated segments). Fix or clear the App Token in Analytics for XR configuration.");
                if (configData.buildType == "production_custom")
                {
                    if (!string.IsNullOrEmpty(configData.appToken))
                    {
                        if (string.IsNullOrEmpty(configData.orgToken))
                            throw new BuildFailedException("Production (Custom APK) requires Organization Token to be set for Custom APK builds. Set the customer's org token in Analytics for XR configuration.");
                        if (!LooksLikeJwt(configData.orgToken))
                            throw new BuildFailedException("Organization Token is set but does not look like a JWT (expected three dot-separated segments). Fix the Organization Token in Analytics for XR configuration.");
                    }
                }
                return;
            }
            if (configData.buildType == "production_custom" && !string.IsNullOrEmpty(configData.appId))
            {
                if (string.IsNullOrEmpty(configData.orgId))
                    throw new BuildFailedException("Production (Custom APK) requires Organization ID to be set for Custom APK builds. Set the customer's org ID in Analytics for XR configuration.");
                if (string.IsNullOrEmpty(configData.authSecret) || string.IsNullOrWhiteSpace(configData.authSecret))
                    throw new BuildFailedException("Production (Custom APK) requires Authorization Secret to be set for Custom APK builds. Set it in Analytics for XR configuration.");
                if (!LooksLikeUuid(configData.orgId))
                    throw new BuildFailedException("Organization ID does not look like a valid UUID (xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx). Fix the Organization ID in Analytics for XR configuration.");
            }
        }
        ...
        private static void ProcessManifest(string projectPath)
        {
            ...
            if (string.IsNullOrEmpty(manifestPath))
            {
                ...
                Logcat.Warning($"AndroidManifest.xml not found. Searched in: {projectPath}");
                return;
            }

            if (!File.Exists(manifestPath))
            {
                Logcat.Warning($"AndroidManifest.xml not found at: {manifestPath}");
                return;
            }
            ...
            {
                Logcat.Error($"Failed to inject metadata into AndroidManifest.xml: {e.Message}\nStackTrace: {e.StackTrace}");
            }
        }
        ...
        private static string GetManifestPath(string projectPath)
        {
            ...
            {
                Logcat.Warning($"Manifest not found. Searched paths:\n{string.Join("\n", possiblePaths)}");
            }
            return null;
        }
        ...
        private static string InsertMetadataAfterApplicationTag(string manifestContent, string metadata)
        {
            ...
            Logcat.Warning("Could not find <application> tag. Metadata not added.");
            return manifestContent;
        }
        ...
        private static string AddCameraPermission(string manifestContent)
        {
            ...
            Logcat.Warning("Could not find <manifest> tag. Camera permission not added.");
            return manifestContent;
        }
        ...
        private static string AddHeadsetCameraPermission(string manifestContent)
        {
            ...
            if (match.Success)
            {
                ...
                Logcat.Info("Added horizonos.permission.HEADSET_CAMERA permission to AndroidManifest.xml");
                return manifestContent;
            }

            Logcat.Warning("Could not find <manifest> tag. Headset camera permission not added.");
            return manifestContent;
        }
        ...
        private static string AddNetworkPermissions(string manifestContent)
        {
            ...
            if (!match.Success)
            {
                Logcat.Warning("Could not find <manifest> tag. Network permissions not added.");
                return manifestContent;
            }
