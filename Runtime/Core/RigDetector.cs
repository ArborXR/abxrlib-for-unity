    ...
    public static class RigDetector
    {
        ...
        private static bool IsTypeInScene(string typeName)
        {
            try
            {
                ...
                {
                    Logcat.Warning($"Generic FindObjectsOfType method failed for {typeName}: {ex.Message}");
                }
                ...
                {
                    Logcat.Warning($"Type-based FindObjectsOfType method failed for {typeName}: {ex.Message}");
                }
                ...
                {
                    Logcat.Warning($"FindObjectsOfType with includeInactive method failed for {typeName}: {ex.Message}");
                }
            ...
            {
                Logcat.Error($"Type detection failed for {typeName}: {ex.Message}");
                return false;
            }
        }
        ...
        private static bool IsTypeInSceneFallback(System.Type targetType)
        {
            try
            {
                ...
                foreach (var gameObject in gameObjects)
                {
                    ...
                    {
                        Logcat.Warning($"Error checking components on GameObject {gameObject.name}: {ex.Message}");
                        continue;
                    }
                }
                return false;
            ...
            {
                Logcat.Error($"Fallback type detection failed: {ex.Message}");
                return false;
            }
        }
        ...
        private static System.Type FindType(string typeName)
        {
            try
            {
                ...
                foreach (var assemblyName in commonAssemblies)
                {
                    ...
                    {
                        Logcat.Warning($"Error searching assembly {assemblyName} for type {typeName}: {ex.Message}");
                    }
                }
                ...
                foreach (var assembly in System.AppDomain.CurrentDomain.GetAssemblies())
                {
                    ...
                    {
                        Logcat.Warning($"Error searching assembly {assembly.GetName().Name} for type {typeName}: {ex.Message}");
                    }
                }
                return null;
            }
            ...
            {
                Logcat.Error($"Type finding failed for {typeName}: {ex.Message}");
                return null;
            }
        }
